...
import { IconAward, IconChartBar, IconEdit, IconSearch, IconUser, IconUserCheck } from '@tabler/icons-react';
  ...
  return (
    <div className="min-h-screen pb-12">
      <div className="max-w-full mx-auto px-6 pb-8">
        {/* Header */}
        <DashboardHeader subtitle="Manage achievements, rewards, and user progress" />
        ...
        {/* Visual Separator - IconSearch */}
        <div className="flex items-center gap-4 mb-10">
          <div className="h-px flex-1 bg-linear-to-r from-transparent via-neutral-200 to-transparent"></div>
          <h2 className="text-xs font-medium text-neutral-500 uppercase tracking-wider">
            IconSearch IconUser Badge IconHistory
          </h2>
          <div className="h-px flex-1 bg-linear-to-r from-transparent via-neutral-200 to-transparent"></div>
        </div>
          ...
          <DashboardCard className="p-5 mb-10">
            <div className="flex gap-3">
              <div className="relative flex-1">
                ...
                <input
                  type="text"
                  placeholder="IconSearch by email, username, or name..."
                  value={searchQuery}
                ...
                <tbody className="divide-y divide-neutral-100">
                  {mockAchievements.map((achievement) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`inline-flex items-center px-2.5 py-1 text-xs font-medium rounded-full border ${getCategoryBadge(achievement.category)}`}
                        >
                          {achievement.category}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-sm font-mono text-neutral-700 bg-neutral-100 px-2 py-1 rounded">
                          {achievement.trigger}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        <div
                          className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full ${achievement.status ? "bg-green-50 border border-green-200" : "bg-neutral-50 border border-neutral-200"}`}
                        >
                          <div
                            className={`w-2 h-2 rounded-full ${achievement.status ? "bg-green-500" : "bg-neutral-400"}`}
                          ></div>
                          <span
                            className={`text-xs font-medium ${achievement.status ? "text-green-700" : "text-neutral-600"}`}
                          >
                      ...
                      <td className="px-6 py-4">
                        <button className="p-2 hover:bg-neutral-100 rounded-lg transition-colors cursor-pointer group">
                          <IconEdit className="w-4 h-4 text-neutral-500 group-hover:text-neutral-700" />
                        </button>
                      </td>
                    </tr>
                  ))}
