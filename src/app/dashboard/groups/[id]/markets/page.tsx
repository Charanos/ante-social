...
import { useMarketList, fetchJsonOrNull } from "@/lib/live-data";
...
export default function GroupMarketsPage() {
  ...
  const { markets: allMarkets, isLoading: isMarketsLoading } = useMarketList();
  ...
  useEffect(() => {
    ...
    const fetchGroup = async () => {
      ...
      const payload = await fetchJsonOrNull<any>(`/api/groups/${groupId}`);
  ...
  // Only include markets that belong to this group
  const groupMarkets = useMemo(() => {
    return allMarkets.filter((m) => m.groupId === groupId);
  }, [allMarkets, groupId]);

  const activeMarkets = useMemo(() => groupMarkets.filter(m => m.status === "active" && new Date(m.endsAt).getTime() > Date.now()), [groupMarkets]);
  const totalPool = groupMarkets.reduce((sum, m) => sum + m.poolAmount, 0);

  const filteredMarkets = useMemo(() => {
    let result = groupMarkets.filter((market) => {
    ...
    return result;
  }, [groupMarkets, searchQuery, filterType, recurringFilter, sortBy]);
  ...
  if (isMarketsLoading || isGroupLoading) return <LoadingLogo fullScreen size="lg" />;
  if (!group) return <div className="p-8 text-sm text-black/50">Group not found.</div>;
    ...
    <div className="min-h-screen pb-12">
      <div className="max-w-full mx-auto md:pl-8 pl-0 pb-8 space-y-2">
        <div className="mb-6 flex items-center gap-4">
          <button
            onClick={() => router.back()}
            className="w-10 h-10 flex cursor-pointer items-center justify-center rounded-full bg-white/40 backdrop-blur-sm border border-black/5 hover:bg-white/60 transition-all text-black/60 hover:text-black"
          >
            <IconArrowLeft className="w-5 h-5" />
          </button>
          <h1 className="text-2xl font-bold tracking-tight text-neutral-900">{Math.min(25, group.name.length) === group.name.length ? group.name : group.name.slice(0, 25) + '...'} Markets</h1>
        </div>
