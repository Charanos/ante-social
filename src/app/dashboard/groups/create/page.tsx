...
import {
    Users, Shield, Globe, Lock, Save,
    Trophy, TrendingUp, Palette, Coffee,
    Image as ImageIcon, Plus, X, ArrowLeft,
    Activity, Brain, ChevronRight, ChevronLeft, LoaderPinwheel
} from "lucide-react"
...
const marketTypes = [
    ...
    { id: "betrayal", name: "Betrayal Game", icon: Shield, description: "Cooperate or betray", color: "red" },
...
const groupCategories = [
    { id: "sports", name: "Sports", icon: Trophy, description: "For athletic competitions", color: "blue" },
    { id: "crypto", name: "Crypto", icon: TrendingUp, description: "Trading and market vibes", color: "green" },
    { id: "lifestyle", name: "Lifestyle", icon: Coffee, description: "Daily life and casual fun", color: "amber" },
    { id: "gaming", name: "Gaming", icon: Palette, description: "Esports and gaming sessions", color: "purple" }
]

const visibilityOptions = [
    { id: "public", name: "Public", icon: Globe, description: "Anyone can find and join", color: "blue" },
    { id: "private", name: "Private", icon: Lock, description: "Join via invitation only", color: "red" }
]
...
export default function CreateGroupPage() {
    ...
    const handleNext = () => {
        ...
        if (currentStep === 2 && (!name || !description)) {
            toast.error("Group Details Required", "Please provide a name and description.")
            return
        }
        if (currentStep === 3 && !selectedMarketType) {
            toast.error("Market Type Required", "Please select a market type for your first bet.")
            return
        }
        if (currentStep === 4 && (!marketTitle || !marketDescription || !buyIn)) {
            toast.error("Market Details Required", "Please fill in all required market fields.")
            return
        }
    ...
    const handleSubmit = () => {
        setIsCreating(true)
        toast.success("Launching Experience...", `Creating ${name} and your first market!`)
                        ...
                        {steps.map((s, i) => (
                            <div key={s.id} className="flex flex-col items-center gap-3 relative z-10">
                                <div
                                    ...
                                    {currentStep > s.id ? <Plus className="w-5 h-5 rotate-45" /> : s.id}
                                </div>
                                <span className={cn(
                ...
                <AnimatePresence mode="wait">
                    {currentStep === 1 && (
                        ...
                        >
                            <div className="flex items-center gap-4 mb-8">
                                ...
                                <h2 className="text-xs font-medium text-neutral-500 uppercase tracking-wider">Select Category</h2>
                                <div className="h-px flex-1 bg-linear-to-r from-transparent via-neutral-200 to-transparent"></div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-10 max-w-7xl mx-auto">
                                {groupCategories.map((cat) => {
                                    ...
                                    const colorMap: Record<string, { bg: string; blur: string; title: string; subtitle: string; icon: string; border: string; activeBorder: string }> = {
                                        blue: {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className="relative z-10 flex flex-col gap-4">
                                                ...
                                                <div>
                                                    <h3 className={cn("font-bold mb-1 transition-colors", colors.title)}>
                                                        {cat.name}
                                                    </h3>
                    ...
                    {currentStep === 2 && (
                        ...
                        >
                            <div className="flex items-center gap-4 mb-10">
                                <div className="h-px flex-1 bg-linear-to-r from-transparent via-neutral-200 to-transparent"></div>
                                <h2 className="text-xs font-medium text-neutral-500 uppercase tracking-wider">Group Identity</h2>
                                <div className="h-px flex-1 bg-linear-to-r from-transparent via-neutral-200 to-transparent"></div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                ...
                                <div className="space-y-6 w-full">
                                    <DashboardCard className="p-6">
                                        <h3 className="text-sm font-semibold text-neutral-900 mb-4 uppercase tracking-widest">Visibility</h3>
                                            ...
                                            {visibilityOptions.map((v) => {
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        `}
                                                    >
                                                        <div className={`p-2 rounded-lg ${isSelected ? `bg-white/20` : 'bg-neutral-50'}`}>
                                                            <Icon className={`w-4 h-4 ${isSelected ? `text-white` : 'text-neutral-500'}`} />
                                                        </div>
                                                        <div>
                                                            <p className="text-xs font-semibold">{v.name}</p>
                                                            <p className={cn("text-[10px]", isSelected ? "text-white/60" : "text-neutral-500")}>{v.description}</p>
                                                        </div>
                                                    </button>
                                                )
                                            })}
                    ...
                    {currentStep === 3 && (
                        ...
                        >
                            <div className="flex items-center gap-4 mb-8">
                                <div className="h-px flex-1 bg-linear-to-r from-transparent via-neutral-200 to-transparent"></div>
                                <h2 className="text-xs font-medium text-neutral-500 uppercase tracking-wider">Choose First Market Type</h2>
                                <div className="h-px flex-1 bg-linear-to-r from-transparent via-neutral-200 to-transparent"></div>
                            </div>
                    ...
                    {currentStep === 4 && (
                        ...
                        >
                            ...
                            <DashboardCard className="p-6 mb-6">
                                <div className="space-y-6">
                                    <div>
                                        <label className="block text-sm font-semibold text-neutral-900 mb-2">Market Title</label>
                                        <input
                                            type="text"
                                            value={marketTitle}
                                    ...
                                    <div>
                                        <label className="block text-sm font-semibold text-neutral-900 mb-2">Market Rules / Description</label>
                                        <textarea
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            <label className="block text-sm font-semibold text-neutral-900 mb-2">Buy-in (KSh)</label>
                                            <input
                                                type="number"
                                                value={buyIn}
                                        ...
                                        <div>
                                            <label className="block text-sm font-semibold text-neutral-900 mb-2">Closing Date</label>
                                            <input
                                                type="datetime-local"
                                                value={closeDate}
                    ...
                    {currentStep === 5 && (
                        ...
                        >
                            ...
                            <h2 className="text-3xl font-semibold text-black mb-4">Launch New Community?</h2>
                            <p className="text-neutral-500 mb-8">
                                You are about to create the group <span className="text-black font-semibold">"{name}"</span> and launch a <span className="text-black font-semibold">"{selectedMarketType}"</span> market.
                            </p>

                            <DashboardCard className="p-6 mb-8 text-left bg-linear-to-br from-neutral-50 to-white">
                                <div className="space-y-4">
                                    <div className="flex justify-between items-center py-2 border-b border-black/5">
                                        <span className="text-sm text-neutral-500 font-semibold uppercase tracking-wider">Group Category</span>
                                        <span className="text-sm font-semibold text-black bg-white px-3 py-1 rounded-full shadow-sm border border-black/5">{selectedCategory}</span>
                                    </div>
                                    <div className="flex justify-between items-center py-2 border-b border-black/5">
                                        <span className="text-sm text-neutral-500 font-semibold uppercase tracking-wider">Visibility</span>
                                        <span className="text-sm font-semibold text-black">{selectedVisibility}</span>
                                    </div>
                                    <div className="flex justify-between items-center py-2 border-b border-black/5">
                                        <span className="text-sm text-neutral-500 font-semibold uppercase tracking-wider">Opening Bet</span>
                                        <span className="text-sm font-semibold text-black">{marketTitle}</span>
                                    </div>
                                    <div className="flex justify-between items-center py-2">
                                        <span className="text-sm text-neutral-500 font-semibold uppercase tracking-wider">Entry Fee</span>
                                        <span className="text-sm font-semibold text-black font-mono">{buyIn} MP</span>
                                    </div>
                                </div>
                            </DashboardCard>
