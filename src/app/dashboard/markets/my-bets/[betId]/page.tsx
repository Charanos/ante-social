...
import { ArrowLeft, Clock, Trophy, X, Save, Trash2, ExternalLink, CheckCircle2, Share2, AlertCircle, ArrowRight, Sparkles, TrendingUp, DollarSign, BarChart3, LoaderPinwheel } from "lucide-react";
...
export default function BetSlipPage() {
        ...
        setTimeout(() => {
            ...
            if (!foundBet && searchParams.get('new') === 'true') {
                foundBet = {
                    ...
                    potentialWin: parseFloat(searchParams.get('amount') || "0") * (Math.random() * 2 + 1.2),
                    type: "poll",
            ...
            if (foundBet) {
                ...
                const foundMarket = mockMarkets.find(m => m.id === foundBet!.marketId) || {
    ...
    const handleUpdateBet = () => {
        setIsEditing(false);
        setBet({ ...bet, amount: newAmount, potentialWin: newAmount * (Math.random() * 2 + 1.2) });
        toast.success("Stake Updated", `New stake amount: $${newAmount}`);
    };

    const handleCancelBet = () => {
        if (confirm("Are you sure you want to cancel this bet?")) {
            toast.info("Bet Cancelled", "Your funds have been returned to your wallet.");
            setTimeout(() => {
                router.back();
            }, 1500)
        }
        ...
        return (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md">
                <div className="relative">
                    <LoaderPinwheel className="w-16 h-16 border-[3px] border-white/10 border-t-white rounded-full animate-spin" />
                </div>
            </div>
        );
    }
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
                ...
                >
                    <div className="bg-black/5 p-6 rounded-2xl w-fit mx-auto">
                        <AlertCircle className="w-12 h-12 text-black/60" />
                    </div>
                    <div className="space-y-2">
                        <p className="text-2xl font-semibold text-black">Bet Slip Not Found</p>
                        <p className="text-sm text-black/50">This bet may have been removed or doesn't exist.</p>
                    </div>
                    <button
    ...
    const StatusIcon = bet.status === 'active' ? Clock :
        bet.status === 'won' ? Trophy : X;
            ...
            >
                            ...
                            <div className="p-4 flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-white/70">Your wager has been placed</p>
                                    </div>
                                </div>
                                <button
                ...
                <div className="bg-black text-white p-6 pb-12 relative overflow-hidden">
                    ...
                    <div className="flex items-start justify-between mb-8 relative z-10">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-[10px] font-semibold text-white/40 uppercase tracking-widest mb-0.5">Status</p>
                                <p className="font-semibold text-lg capitalize tracking-tight">{bet.status}</p>
                            </div>
                        </div>
                    ...
                    <div className="relative z-10 space-y-2 max-w-[90%]">
                        <div className="flex items-center justify-between">
                            <h2 className="text-2xl font-semibold leading-snug tracking-tight">{bet.title}</h2>

                        </div>
                        <div className="flex items-center justify-between gap-2 text-white/40 text-xs font-normal">
                            <div className="flex items-center gap-2">
                                <Clock className="w-3.5 h-3.5" />
                                <span>{new Date(bet.date).toLocaleDateString(undefined, { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>
                            </div>

                            <div className="flex items-center gap-2">
                                <p className="text-xs font-semibold text-white/30 uppercase tracking-widest mb-0.5">Bet ID: </p>
                                <p className="font-mono text-xs text-white/60">#{bet.id.slice(0, 8)}</p>
                            </div>
                        </div>
                ...
                <div className="relative -mt-6 mx-6 z-20">
                    <div className="bg-white border border-black/5 rounded-2xl p-5 shadow-[0_8px_30px_rgb(0,0,0,0.06)] flex items-center justify-between gap-4">
                        <div className="flex-1">
                            <p className="text-[10px] font-semibold text-black/40 uppercase tracking-widest mb-1.5">You Picked</p>
                            <p className="text-xl font-semibold text-black leading-tight">{bet.outcome}</p>
                        </div>
                        <div className="w-px h-10 bg-black/5" />
                        <div className="shrink-0 text-right">
                            <p className="text-[10px] font-semibold text-black/40 uppercase tracking-widest mb-1.5">Type</p>
                            <div className="bg-black/5 px-3 py-1 rounded-full">
                                <p className="text-xs font-semibold text-black/70 capitalize">{bet.type || 'Standard'}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-6 pt-6 space-y-8">
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center justify-between">
                            <p className="text-xs font-semibold text-black/40 uppercase tracking-wider">Stake Amount</p>
                        ...
                        {isEditing ? (
                            <div className="space-y-3">
                                <div className="relative group">
                                    <span className="absolute left-4 top-1/2 -translate-y-1/2 text-2xl font-semibold text-black/20 group-focus-within:text-black transition-colors">$</span>
                                    <input
                                        type="number"
                                        value={newAmount}
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <button
                                        onClick={() => { setIsEditing(false); setNewAmount(bet.amount); }}
                    ...
                    <div className="space-y-4 pt-6 border-t border-black/5 border-dashed">
                        ...
                        {bet.status !== 'won' && (
                            <div className="flex items-center justify-between">
                                <span className="text-sm font-medium text-black/40">Potential Profit</span>
                        ...
                        <div className="flex items-center justify-between p-4 rounded-2xl bg-linear-to-br from-gray-50 to-white border border-black/5 shadow-sm">
                            <div className="flex items-center gap-3">
                                <div className={cn("p-2 rounded-full", bet.status === 'won' ? "bg-green-100" : "bg-black/5")}>
                                    {bet.status === 'won' ? <CheckCircle2 className="w-4 h-4 text-green-600" /> : <TrendingUp className="w-4 h-4 text-black" />}
                                </div>
                            ...
                            <span className={cn(
                                ...
                                ${bet.potentialWin.toLocaleString(undefined, { maximumFractionDigits: 2 })}
                            </span>
                        </div>
                    </div>
