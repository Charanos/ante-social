...
export default function DivergenceMarketPage() {
  ...
  const handlePlaceBet = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(parseApiError(payload, "Failed to place vote."));
        ...
        <div className="lg:col-span-8 space-y-8">
            ...
            >
              <div className="flex items-center gap-3 mb-8 pb-6 border-b border-slate-100">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-500 font-semibold uppercase tracking-widest mt-0.5">Final distribution of community consensus</p>
                </div>
              </div>
          ...
          {!isClosed && (
            <div className="flex items-center gap-4 my-10 md:my-16">
              ...
              <h2 className="text-xs font-medium text-neutral-500 uppercase tracking-widest">Select Your Choice</h2>
              <div className="h-px flex-1 bg-gradient-to-r from-transparent via-neutral-200 to-transparent" />
            </div>
          )}
                    ...
                    >
                      ...
                      <div className="p-5 space-y-4">
                        <div className="flex items-start justify-between gap-3">
                          <h4 className="text-base font-medium text-slate-900 leading-tight flex-1">{option.option_text}</h4>
                          <span className="text-lg font-semibold font-mono text-slate-700 shrink-0">{option.percentage}%</span>
                        </div>
                        <div className="space-y-2.5">
                          <div className="h-2 bg-slate-100 rounded-full overflow-hidden shadow-inner">
                            <motion.div
                              ...
                              className={`h-full rounded-full ${isWinner ? "bg-emerald-500" : isSelected ? "bg-slate-900" : "bg-slate-400"}`}
                            />
                          </div>
                          <div className="flex justify-between items-center">
                            <span className="text-xs text-slate-500 font-normal">{option.votes} votes</span>
                            {isSelected && !isWinner && (
                              <span className="text-xs text-slate-900 font-semibold tracking-wide">SELECTED</span>
                            )}
                            {isWinner && (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500 font-normal mt-0.5">Secured prediction</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-sm font-semibold font-mono text-slate-900">{formatCurrency(participant.total_stake || 0)}</p>
            ...
            {isClosed ? (
                ...
                <div className="p-6 space-y-4">
                  ...
                  <div className="flex flex-col gap-3 mt-4">
                    <Link href={`/dashboard/markets/my-forecasts/${predictionResult.transactionId}?new=true`} className="w-full">
                      <button className="w-full py-2 tracking-wider bg-slate-900 text-white rounded-xl font-medium cursor-pointer hover:bg-gray-600 transition-all shadow-md shadow-black/20">
                        View in My Predictions
                      </button>
                    </Link>
                    <button
                      onClick={() => setPredictionResult(null)}
                      className="w-full py-2 tracking-wider bg-slate-100 text-slate-700 rounded-xl font-medium cursor-pointer hover:bg-slate-200 transition-all"
                    >
                      Dismiss
                    </button>
                  </div>
              ...
              >
                <div className="p-6 bg-slate-900 text-white">
                  ...
                  <p className="text-sm text-white/60 font-normal">Join the poll and win</p>
                </div>
