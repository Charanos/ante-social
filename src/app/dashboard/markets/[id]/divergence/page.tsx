                    ...
                    <div className="space-y-4">
                      {market.options.map((opt: any, idx: number) => (
                        <div key={idx} className="flex items-center justify-between p-4 rounded-2xl bg-slate-50/50 border border-slate-100">
                          ...
                          <span className="text-sm font-bold font-mono text-slate-900">{opt.percentage}%</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  >
                    ...
                    <div className="p-6 space-y-4">
                      ...
                      <div className="flex justify-between items-center py-2.5 border-b border-slate-100">
                        ...
                        <span className="text-base font-bold font-mono text-slate-900">{formatCurrency(market.total_pool)}</span>
                      </div>
                      <div className="flex justify-between items-center py-2.5 border-b border-slate-100">
                        <span className="text-sm text-slate-500 font-medium">Total Players</span>
                        <span className="text-base font-semibold font-mono text-slate-900">{market.participant_count}</span>
                      </div>
                    ...
                    <div className="p-2 space-y-1">
                      {sortedParticipants.map((p: any, i: number) => (
                        <div key={i} className="flex items-center justify-between p-3 rounded-xl hover:bg-slate-50 transition-colors">
                          <div className="flex items-center gap-3">
                            <span className={`text-xs font-bold w-4 text-center ${i < 3 ? 'text-slate-900' : 'text-slate-400'}`}>
                              {i + 1}
                            </span>
                            <UserAvatar name={p.username} size="sm" border={false} />
                            <span className="text-sm font-semibold text-slate-700">{p.username}</span>
                          </div>
                          <span className="text-sm font-bold font-mono text-slate-900">{formatCurrency(p.total_stake)}</span>
                        </div>
                      ))}
                  ...
                  >
                    ...
                    <div className="p-6 space-y-4">
                      <div className="flex justify-between items-center py-2 border-b border-slate-100">
                        <span className="text-sm text-slate-500 font-medium">Outcome</span>
                        <span className="text-base font-bold text-slate-900">{predictionResult.optionText}</span>
                      </div>
                      <div className="flex justify-between items-center py-2 border-b border-slate-100">
                        <span className="text-sm text-slate-500 font-medium">Stake</span>
                        <span className="text-base font-bold font-mono text-slate-900">{formatCurrency(predictionResult.amount)}</span>
                      </div>
                      <div className="flex justify-between items-center py-2">
                        <span className="text-sm text-slate-500 font-medium">Platform Fee</span>
                        <span className="text-sm font-medium font-mono text-slate-600">{formatCurrency(predictionResult.amount * 0.05)}</span>
                      </div>
                      <button 
                        onClick={() => setPredictionResult(null)}
                        className="w-full py-3.5 bg-slate-900 text-white rounded-xl font-bold hover:bg-slate-800 transition-all shadow-xl shadow-slate-900/20 mt-4"
                      >
                        Dismiss
                      </button>
                    </div>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="relative">
                          <input
                            ...
                            className="w-full px-4 py-3 pr-16 bg-white border border-slate-200 rounded-xl text-lg font-mono font-semibold text-slate-900 focus:border-slate-400 focus:ring-4 focus:ring-slate-100 outline-none transition-all placeholder:text-slate-300"
                          />
