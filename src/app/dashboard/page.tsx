...
function LiveWinCard({ win }: any) {
    return (
        <div className="w-[240px] shrink-0 px-2">
            <div className="flex items-center gap-3 px-4 py-3 bg-white/60 backdrop-blur-sm rounded-2xl shadow-sm border border-black/5 hover:bg-white/80 hover:border-black/10 hover:shadow-md transition-all cursor-pointer group">
                ...
                <div className="flex flex-col flex-1 min-w-0">
                    <span className="text-sm font-semibold text-black/90 truncate">{win.name}</span>
                    <span className="text-xs text-black/50 font-medium truncate">{win.game}</span>
                </div>
    ...
    return (
        <div className="space-y-10 pb-12 pl-8 overflow-x-hidden w-full max-w-[100vw]">
            ...
            <motion.div
                ...
                className="relative overflow-visible rounded-3xl bg-linear-to-br from-neutral-900 via-neutral-800 to-black text-white shadow-2xl p-8 md:p-10 border border-white/10 mb-10"
            >
                    ...
                    <div className="flex-1 space-y-6">
                        ...
                        <div className="flex gap-4 pt-2">
                            <Link href="/dashboard/profile/achievements">
                                <motion.button
                                    className="bg-white/10 cursor-pointer hover:bg-white/20 border border-white/10 text-white backdrop-blur-md rounded-xl px-6 py-3 text-base font-medium tracking-wider shadow-lg transition-all flex items-center gap-2"
                                    whileHover={{ scale: 1.05 }}
                                    whileTap={{ scale: 0.95 }}
                                >
                                    <DicesIcon className="w-5 h-5 text-amber-400" /> Daily Bonus
                                </motion.button>
                            </Link>
                            <Link href="/dashboard/wallet">
                                <motion.button
                                    className="bg-linear-to-r cursor-pointer from-amber-500 to-yellow-500 hover:brightness-110 text-black border-0 rounded-xl px-8 py-3 text-base font-medium tracking-wider shadow-[0_0_20px_rgba(245,158,11,0.3)] transition-all"
                                    whileHover={{ scale: 1.05 }}
                                    whileTap={{ scale: 0.95 }}
                                >
                                    Deposit
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between px-2">
                    <div className="flex w-full gap-6 border-b border-black/5">
                        {["All Markets", "Recurring", "One-Time"].map((tab) => (
                            <button
                                key={tab}
                                onClick={() => { setActiveTab(tab); setCurrentPage(1); }}
                                className={`pb-3 text-sm cursor-pointer font-semibold transition-all relative ${activeTab === tab ? "text-black/90" : "text-black/40 hover:text-black/60"
                                    }`}
                            >
                                {tab}
                                {activeTab === tab && (
                ...
                {isReadMeOpen && (
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="space-y-6">
                                <div>
                                    <h3 className="text-2xl font-semibold text-black/90 mb-2">How to Play</h3>
                                ...
                                <div className="space-y-4">
                                    {[
                                        { step: 1, text: "Browse active markets and choose your game mode" },
                                        { step: 2, text: "Select an outcome and place your stake (KSH or USD)" },
                                        { step: 3, text: "Wait for market settlement - results are instant" },
                                        { step: 4, text: "Winners split the prize pool pro-rata by stake" },
                                    ].map((item) => (
                                ...
                                <div className="pt-6 border-t border-black/5">
                                    <div className="flex items-start gap-3 p-4 bg-black/5 rounded-2xl">
                                        ...
                                        <div>
                                            <p className="text-sm font-semibold text-black/90 mb-1">Pro Tip</p>
                                            <p className="text-xs text-black/60 font-medium leading-relaxed">
                                                Smaller groups often mean higher payouts. Be contrarian, think different, win big.
                                            </p>
                                        </div>
                                    </div>
                                </div>
