...
import { Trophy, Target, Activity, ListOrdered, ChevronRight, ChevronLeft, Plus, Image as ImageIcon, Calendar, DollarSign, Info, X, Users, LayoutGrid, Shield } from "lucide-react"
...
export function MarketCreationWizard() {
    ...
    const handleSubmit = async () => {
        ...
        const label = isCreatingGroup ? "Creating Group & Market..." : "Creating Market..."
        toast.loading(label, "Broadcasting to the network")

        // Simulate API
        setTimeout(() => {
            setIsSubmitting(false)
            const successMsg = isCreatingGroup ? "Group & Market Launched!" : "Market Launched!"
            toast.success(successMsg, "Redirecting to your new group...")
            const redirectPath = isCreatingGroup ? "/dashboard/groups/group1" : "/dashboard/markets/market1"
        ...
        <div className="w-full max-w-5xl mx-auto">
            ...
            <div className="mb-12">
                <div className="flex items-center justify-between relative z-10">
                    {activeSteps.map((s, i) => (
                        <div key={s.id} className="flex flex-col items-center gap-3 relative z-10 w-40">
                            <div
                                onClick={() => i < activeSteps.findIndex(step_obj => step_obj.id === step) && setStep(s.id)}
                                className={cn(
                                    ...
                                    s.id < step && "cursor-pointer hover:scale-110 active:scale-95"
                                )}>
                                {step > s.id ? <Activity className="w-5 h-5" /> : s.id}
                            </div>
                            <span className={cn(
            ...
            <div className="glass-panel p-8 md:p-12 rounded-[2.5rem] border-white/60 bg-white/70 shadow-xl shadow-gray-200/50 min-h-[600px] relative overflow-hidden">
                ...
                <form onSubmit={e => e.preventDefault()} className="relative z-10 h-full flex flex-col justify-between">
                    <AnimatePresence mode="wait">
                        {step === 0 && (
                            <motion.div key={0} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8 max-w-2xl mx-auto w-full">
                                <div className="text-center space-y-2 mb-10">
                                    <h2 className="text-2xl font-semibold text-gray-900">Establish Your Group</h2>
                                    <p className="text-gray-500">You'll be the group's first admin</p>
                                </div>

                                <div className="space-y-6">
                                    <div className="space-y-2">
                                        <label className="text-sm font-medium text-gray-900 ml-1">Group Name</label>
                                        <input
                                            type="text"
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-sm font-medium text-gray-900 ml-1">About this Group</label>
                                        <textarea
                                            rows={4}
                                    ...
                                    <div className="p-6 rounded-3xl bg-neutral-50 border border-neutral-100 flex items-center justify-between">
                                        <div className="flex items-center gap-4">
                                            <div className={cn(
                                                ...
                                                formData.isPublic ? "bg-blue-100 text-blue-600" : "bg-neutral-200 text-neutral-600"
                                            )}>
                                                {formData.isPublic ? <LayoutGrid className="w-5 h-5" /> : <Shield className="w-5 h-5" />}
                                            </div>
                                            <div>
                                                <p className="text-sm font-semibold text-gray-900">{formData.isPublic ? "Public Group" : "Private Group"}</p>
                                    ...
                                    <div className="p-4 rounded-2xl bg-amber-50 border border-amber-100 flex gap-4 items-center">
                                        ...
                                        <p className="text-sm text-amber-800 font-medium">As creator, you'll have full control over members and market resolution.</p>
                                    </div>
                                </div>
                            </motion.div>
                        )}
                        ...
                        {step === 1 && (
                            <motion.div key={1} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                                <div className="text-center space-y-2 mb-10">
                                    <h2 className="text-2xl font-semibold text-gray-900">Choose Market Type</h2>
                                    <p className="text-gray-500">Select the mechanism for this betting market</p>
                                </div>

                                <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
                                    {[
                                        { id: 'poll', label: 'Poll', icon: Trophy, desc: 'Multiple options, winner takes all.' },
                                        { id: 'betrayal', label: 'Betrayal', icon: Target, desc: "Prisoner's dilemma style game." },
                                        { id: 'reflex', label: 'Reflex', icon: Activity, desc: 'Time-sensitive prediction windows.' },
                                        { id: 'ladder', label: 'Ladder', icon: ListOrdered, desc: 'Rank items in correct order.' },
                                    ].map((type) => (
                                        <button
                                            key={type.id}
                                            onClick={() => updateForm({ type: type.id as MarketType })}
                                            className={cn(
                                        ...
                                        >
                                            <div className={cn(
                                                ...
                                                formData.type === type.id ? "bg-white/20 text-white" : "bg-gray-100 text-gray-900"
                                            )}>
                                                <type.icon className="w-6 h-6" />
                                            </div>
                                            <h3 className="font-medium text-lg mb-2">{type.label}</h3>
                                            <p className={cn("text-sm leading-relaxed", formData.type === type.id ? "text-white/60" : "text-gray-500")}>
                                                {type.desc}
                                            </p>
                                        </button>
                                    ))}
                        ...
                        {step === 2 && (
                            <motion.div key={2} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8 max-w-2xl mx-auto w-full">
                                <div className="space-y-6">
                                    <div className="space-y-2">
                                        <label className="text-sm font-medium text-gray-900 ml-1">Market Title</label>
                                        <input
                                            type="text"
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="space-y-10">
                                            <label className="text-sm font-medium text-gray-900 ml-1 ">Category</label>
                                            <select
                                        ...
                                        {!isCreatingGroup && mockUser.role === 'group_admin' && (
                                            <div className="space-y-2">
                                                <label className="text-sm font-medium text-gray-900 ml-1">Post To</label>
                                                <div className="relative">
                                                    <Users className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 w-4 h-4 pointer-events-none" />
                                                    <select
                                                    ...
                                                    >
                                                        ...
                                                        {mockGroups.filter(g => mockUser.managed_groups?.includes(g.id)).map(group => (
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-sm font-medium text-gray-900 ml-1">Description</label>
                                        <textarea
                                            rows={3}
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-sm font-medium text-gray-900 ml-1">Cover Image URL</label>
                                        <div className="relative">
                        ...
                        {step === 3 && (
                            <motion.div key={3} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8 max-w-2xl mx-auto w-full">
                                {/* Financials */}
                                <div className="grid grid-cols-2 gap-6 bg-gray-50 p-6 rounded-3xl border border-gray-100">
                                    <div className="space-y-2">
                                        <label className="text-xs font-medium uppercase tracking-wider text-gray-500 ml-1">Buy-In</label>
                                        <div className="relative">
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-xs font-medium uppercase tracking-wider text-gray-500 ml-1">End Date</label>
                                        <div className="relative">
                                            <input
                                                type="date"
                                ...
                                {formData.type === 'poll' && (
                                    <div className="space-y-4">
                                        <div className="flex items-center justify-between">
                                            <label className="text-sm font-medium text-gray-900">Poll Options</label>
                                            <button
                                                onClick={() => updateForm({ options: [...(formData.options || []), { text: "", image: "" }] })}
                                ...
                                {formData.type !== 'poll' && (
                                    <div className="p-6 rounded-2xl bg-blue-50 border border-blue-100 flex gap-4 items-center">
                                        ...
                                        <p className="text-sm text-blue-800">Advanced configuration for {formData.type} markets coming soon. Standard defaults will apply.</p>
                                    </div>
                                )}
                            </motion.div>
                        )}
                        ...
                        {step === 4 && (
                            <motion.div key={4} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8 max-w-2xl mx-auto w-full text-center">
                                <div className="w-20 h-20 bg-green-100 text-green-600 rounded-full flex items-center justify-center mx-auto mb-6">
                                    <Trophy className="w-10 h-10" />
                                </div>
                                <h2 className="text-3xl font-semibold text-gray-900">Ready to Launch?</h2>
                                <div className="space-y-1">
                                    <p className="text-gray-500 max-w-md mx-auto">
                                        You are about to create {isCreatingGroup ? <>the group <strong>{formData.groupName}</strong> and </> : 'a '}
                                        <strong>{formData.type}</strong> market in <strong>{formData.category}</strong>.
                                    </p>
                                    <p className="text-gray-400 text-sm">
                                        {isCreatingGroup ? (
                                            <span className="flex items-center justify-center gap-1">
                                                <Shield className="w-3 h-3" /> You will be the group admin
                                            </span>
                                        ) : (
                                            <>Target: {formData.targetGroup === 'global' ? 'Global Feed' : `Group ID: ${formData.targetGroup}`}</>
                                        )}
                                    </p>
                                </div>
                                ...
                                <div className="p-4 rounded-2xl bg-gray-50 border border-gray-200 text-left max-w-md mx-auto space-y-2">
                                    <div className="flex justify-between text-sm">
                                        ...
                                        <span className="font-mono font-normal">${formData.buyIn || '0'}</span>
                                    </div>
                                    <div className="flex justify-between text-sm">
                                    ...
                                    <div className="flex justify-between text-sm pt-2 border-t border-gray-100">
                                        ...
                                        <span className="font-mono font-normal text-green-600">Immediate</span>
                                    </div>
                                </div>
                            </motion.div>
                        )}
                    ...
                    <div className="flex items-center justify-between pt-8 mt-8 border-t border-gray-100">
                        ...
                        >
                            {step === 4 ? (isSubmitting ? "Launching..." : "Launch Market") : "Continue"}
