...
export function Sidebar({ collapsed = false, onToggle = () => { }, user }: SidebarProps) {
  ...
  return (
    ...
    >
      ...
      <button
        onClick={onToggle}
        className="absolute -right-3 top-9 z-50 flex h-6 w-6 cursor-pointer items-center justify-center rounded-full border border-white/50 bg-white/80 backdrop-blur-md text-gray-500 shadow-sm transition-all hover:bg-white hover:text-gray-900 hover:scale-110"
      ...
      <div className={cn(
        ...
        <ul className="space-y-1.5 flex-1">
          {navItems.filter(item => item.title !== "Admin" || (session?.user as any)?.role === "admin").map((item) => {
              ...
              <li key={item.title}>
                <div
                  className={cn(
                    ...
                    isActive && !hasChildren
                      ...
                      : 'text-gray-500 hover:bg-white/50 hover:shadow-sm hover:text-gray-900',
                ...
                >
                  ...
                  {hasChildren ? (
                    ...
                    <Link
                      href={item.url}
                      className={cn("flex w-full items-center", collapsed && "justify-center")}
                      title={collapsed ? item.title : undefined}
                    >
                      <div className="relative shrink-0">
                        <item.icon className={cn(
                          "h-5 w-5 transition-colors duration-200",
                          isActive && !hasChildren ? 'text-white' : 'text-gray-400 group-hover:text-gray-600',
                      ...
                      {!collapsed && (
                        <div className="flex flex-1 items-center justify-between ml-3 overflow-hidden">
                          <span className={cn(
                            "text-sm font-medium tracking-wide transition-opacity duration-200 truncate",
                            isActive && !hasChildren ? "text-white" : "text-gray-600 group-hover:text-gray-900"
                          )}>
                            {item.title}
                          </span>
                          {isNotifications && unreadCount > 0 && (
