...
// Related- `role`: String (enum: `admin`, `group_admin`, `user`)

- `wallet`: Object (balance, total_deposits, etc.)
- `managed_groups`: Array of ObjectIds (Ref: `groups`, only for `group_admin`)
...
- `title`: String
- `description`: String
- `type`: String (enum: `poll`, `betrayal`, `reflex`, `ladder`)
- `category`: String (e.g. `Sports`, `Politics`)
- `groupId`: ObjectId (Optional, Ref: `groups`)
...
adminId: ObjectId("..."),
createdBy: ObjectId("..."), // Same as adminId on initialization
inviteCode: "FN-CREW-2024",
isPublic: true,

// New: Integrated Initial Market
initialMarketId: ObjectId("..."),
