...
async function settlePollMarket(marketId: string) {
  ...
  // Find winning option
  const winningOptionId = Object.keys(voteCounts)
    .reduce((a, b) => voteCounts[a] > voteCounts[b] ? a : b)
  
  // Get winners
  const winners = bets.filter(bet => bet.option_id === winningOptionId)
  const totalWinnerStakes = winners.reduce((sum, bet) => sum + bet.stake_amount, 0)
  ...
  for (const winner of winners) {
    ...
    await createTransaction({
      ...
      amount: payoutShare,
      market_id: marketId,
      metadata: { total_pool, platform_fee_collected: platform_fee, prize_pool }
    })
  }
...
async function settleBetrayalMarket(marketId: string) {
  ...
  // Majority Cooperate, Minority Betray
    ...
    winners = betrayers
    const totalBetrayerStakes = betrayers.reduce((sum, bet) => sum + bet.stake_amount, 0)
    for (const winner of winners) {
...
async function settleReflexMarket(marketId: string) {
  ...
  const choiceCounts = {}
  for (const bet of bets) {
    choiceCounts[bet.predicted_option] = (choiceCounts[bet.predicted_option] || 0) + 1
  }
  
  const majorityOption = Object.keys(choiceCounts)
    .reduce((a, b) => choiceCounts[a] > choiceCounts[b] ? a : b)
...
async function settleLadderMarket(marketId: string) {
  ...
  const majorityRankingKey = Object.keys(rankingCounts)
    .reduce((a, b) => rankingCounts[a] > rankingCounts[b] ? a : b)
  
  const majorityRanking = majorityRankingKey.split("→")
  
  // Winners: exact matches only
  const winners = bets.filter(bet => {
    return JSON.stringify(bet.ranked_option_ids) === JSON.stringify(majorityRanking)
  })
